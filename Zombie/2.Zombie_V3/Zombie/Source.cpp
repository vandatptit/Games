...
void slash(int x, int y, int d){
	...
	case 1:{
				...
				if (MAP[x-1][y]=='*') setMap(x-1,y,'0');
			break;
		   }
	case 2:{
			
				if (MAP[x][y+1]=='*') setMap(x,y+1,'0');
			break;
		   }
	case 3:{
...
void move(Player &player, int direction){
	...
	if(MAP[player.getLocation().getX()+dx[direction]][player.getLocation().getY()+dy[direction]]=='0'){
		...
		setMap(player.getLocation().getX(),player.getLocation().getY(),'0');
		player.updateLocation(direction);

		char test = player.getDisplay();

		setMap(player.getLocation().getX(),player.getLocation().getY(),test);
	}
}

void moveZ(Zombie *zombie, int x, int y){
	setMap(zombie->getLocation().getX(),zombie->getLocation().getY(),'0');
	zombie->setLocation(x,y);
	setMap(zombie->getLocation().getX(),zombie->getLocation().getY(),zombie->getDisplay());
}
...
void randomZombie2(){
	...
	while(true){
		...
		while(MAX > 0 && count > 0){
			...
			int* zombieWay = BFS2((char*)MAP,25,45,x,y,xPlayer,yPlayer,xPlayer2,yPlayer2,length);
			if(length > 20){
				thread dfs (threadZombie2,x,y);
				dfs.detach();
				dfs.~thread();
				break;
			}
